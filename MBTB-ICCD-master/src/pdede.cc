...
uint64_t PDEDE_BTB::get_tag(uint64_t addr, uint8_t btb_type) {
  ...
  //here using 12 bit tag need to extracted
    ...
    /////////////////////////////////////////////////////////////////////////
    // addr >>= LOG2_L2BTB_SET; //first 12 bit is for indexing 
    // addr = addr & ((1L << NUM_TAG_BIT) - 1); // next 20 bit for tagging
    // return addr;


    return addr & ((1L << NUM_TAG_BIT)-1);
    /////////////////////////////////////////////////////////////////////////
  } 
...
uint32_t PDEDE_BTB::get_set(uint64_t address, uint8_t btb_type) {
  ...
  if (btb_type == IS_L1BTB) {
    ...
    // address = (uint32_t)(address & ( (1L << LOG2_L2BTB_SET) -1));  //first 12 bit 
    // return address;
...
uint64_t PDEDE_BTB::reconstruct_target_data(uint64_t set, uint64_t way){

            //this data contains pointer to region, page and offset
            uint64_t data = L2BTB.block[set][way].data;
