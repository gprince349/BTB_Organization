...
#define L1BTB_SET 128 // this is number of entry for out BTB_monitor
#define L1BTB_WAY 2
...
//need to find equivalence between baseline btb size and pdede size 
//here using region and page we've reduced storage requirement so we can leverage this and
//increase the overall number of sets to match total size to baseline BTB
...
//////////////////Classes for PDEDE Architecture////////

class REGION {
    public:
    uint64_t data; //29bit region part
    uint64_t srrip; //3 bit srrip bit
...
public:
  ...
  //for L1 BTB adding regions and pages to store part of the address to implement partitioning and deduplication
  ...
  uint64_t reconstruct_target_data(uint64_t address, uint64_t set, uint64_t way);
